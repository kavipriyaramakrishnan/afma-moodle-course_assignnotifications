...
if ($mform->is_cancelled()) {
    ...
    // Added by Pukunui - VFK-574585
    if (!strcmp($fromform->modulename, 'assign')) {
        if (!empty($fromform->thirdsubmission)) {
            $assignid = $fromform->instance;
            $records = new stdClass();
            $records->assignid = $assignid;
            $records->notifyemail = $fromform->thirdsubmission;
            if ($rec = $DB->get_record('assign_notifyemail', array('assignid' => $assignid))) {
                // Update
                $records->id = $rec->id;
                $DB->update_record('assign_notifyemail', $records);
            } else {
                // Insert
                $DB->insert_record('assign_notifyemail', $records);
            }
        }
    }
    // End of changes by Pukunui - VFK-574585
